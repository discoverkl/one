...
const (
	b32StdPadding rune = 'e' // Standard padding character
	b32NoPadding  rune = -1  // No padding
)

const encodeStd = "abcdfghjklmnpqrstvwxyz0123456789"

// newB32Encoding returns a new Encoding defined by the given alphabet,
// which must be a 32-byte string.
func newB32Encoding(encoder string) *b32Encoding {
	if len(encoder) != 32 {
...
// b32StdEncoding is the standard base32 encoding, as defined in
// RFC 4648.
var b32StdEncoding = newB32Encoding(encodeStd)
...
// below '\xff'.
func (enc b32Encoding) WithPadding(padding rune) *b32Encoding {
...
func (enc *b32Encoding) Encode(dst, src []byte) {
	for len(src) > 0 {
		var b [8]byte
...
func (enc *b32Encoding) EncodeToString(src []byte) string {
...
// newB32Encoder returns a new base32 stream encoder. Data written to
// the returned writer will be encoded using enc and then written to w.
// Base32 encodings operate in 5-byte blocks; when finished
// writing, the caller must Close the returned encoder to flush any
// partially written blocks.
func newB32Encoder(enc *b32Encoding, w io.Writer) io.WriteCloser {
	return &encoder{enc: enc, w: w}
}

// EncodedLen returns the length in bytes of the base32 encoding
// of an input buffer of length n.
func (enc *b32Encoding) EncodedLen(n int) int {
	if enc.padChar == b32NoPadding {
		return (n*8 + 4) / 5
	}
	return (n + 4) / 5 * 8
...
func (enc *b32Encoding) decode(dst, src []byte) (n int, end bool, err error) {
...
func (enc *b32Encoding) Decode(dst, src []byte) (n int, err error) {
...
func (enc *b32Encoding) DecodeString(s string) ([]byte, error) {
	buf := []byte(s)
...
// newB32Decoder constructs a new base32 stream decoder.
func newB32Decoder(enc *b32Encoding, r io.Reader) io.Reader {
	return &decoder{enc: enc, r: &newlineFilteringReader{r}}
}

// DecodedLen returns the maximum length in bytes of the decoded data
// corresponding to n bytes of base32-encoded data.
func (enc *b32Encoding) DecodedLen(n int) int {
	if enc.padChar == b32NoPadding {
		return n * 5 / 8
	}
